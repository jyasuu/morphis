...
| `default` | string | `null` | SQL default expression for the column (e.g. `'active'`) |
| `prompt` | string | `null` | Natural language description of the column, used by MCP `discover_tables` |
| `examples` | string[] | `null` | Example values for the column, used by MCP `discover_tables` |
...
**MCP metadata** (optional, for AI assistants):

```yaml
tables:
  materials:
    table: materials
    prompt: "Product catalog of raw materials used in manufacturing."
    common_queries:
      - description: "Find materials by feature name"
        tool: graphql
        params: { query: "{ material_featuresList(filter: {feature_name: \"Water Resistant\"}) { mat_no material { mat_no name } } }" }
```

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `prompt` | string | `null` | Natural language description of the table, used by MCP `discover_tables` |
| `common_queries` | array | `[]` | Predefined query examples with descriptions, used by MCP `discover_tables` |
...
relations:
  - name: items
    type: has_many
    table: order_items
    local_fields: [order_id, order_seq]
...
# No table in the current config uses composite PKs; conceptually:
# { orderItems(order_id: "ORD001", line_no: 1) { order_id line_no quantity } }
...
# mutation { updateOrderItems(order_id: "ORD001", line_no: 1, input: { quantity: 5 }) { order_id line_no } }
# mutation { deleteOrderItems(order_id: "ORD001", line_no: 1) { order_id line_no } }
```
...
| Key | Purpose |
|-----|---------|
| `defaultFilterComponent` | Default filter UI for all entities (`advanced` or `simple`) |
| `defaultPermissions` | Default CRUD permissions for all entities |
...
| `relationFilters[].defaultLogic` | Default conjunction logic (`and`/`or`) for multi-value relation filters |
