...
Test order: health → mutations → **cleanup** → queries → relations → search → row_filters → auth_proxy → mcp → **re-seed**
...
| `tests/mutations.hurl` | CRUD (creates orphan `HL` size — cleaned up after). Also tests duplicate PK and non-existent ID errors |
| `tests/queries.hurl` | List/single queries on seed data — includes pagination (limit/offset/beyond-total), nullable field reads, `__schema` introspection |
...
| `tests/mcp.hurl` | MCP endpoint reachability through auth-proxy (with HS256 JWT) and direct without JWT (expects 401) |
...
**Script** (recommended — handles env vars, cleanup, and process group kill):
```bash
bash scripts/run-frontend-tests.sh              # OIDC auth-chain tests
bash scripts/run-frontend-tests.sh --credentials # credentials mode (crud + entities)
```

⚠ The `--credentials` flag clears `.next/` cache and unsets OIDC env vars to prevent cross-mode leakage. Both modes are wired into `run-all.sh` as steps 2a (OIDC) and 2b (credentials).
...
- **`rm: cannot remove ... turbopack: Directory not empty`**: Zombie `next-server` child processes (build workers) can hold open file handles in `.next/dev/cache/turbopack/`, preventing `rm -rf .next` from completing. Kill ALL `next-server` and `node .next/dev/build` processes before cleaning the cache. Use `ss -tlnp | grep 3000` to find the main server PID, then `ps --ppid <PID> -o pid=` to find children, or `kill -9` with process group: `kill -- -$(ps -o pgid= -p <PID> | tr -d ' ')`.
...
| `NEXT_PUBLIC_GRAPHQL_URL` | `http://localhost:3000/api/graphql` | `http://localhost:3000/api/graphql` |
| `NEXT_PUBLIC_AUTH_OIDC_NAME` | `Keycloak` | — (must be unset to hide OIDC button) |
...
|---|---|---|
| `./scripts/run-all.sh` | Full suite from zero (build → backend → frontend both modes) | After any changes |
...
| `./scripts/run-all.sh --playwright-oidc` | Full suite, but frontend OIDC only (skip credentials mode) | After OIDC auth changes only |
...
| `bash scripts/run-frontend-tests.sh --credentials` | Frontend credentials-mode tests (crud + entities) | After changing CRUD or entity workflows |
