        ...
        match rel.rel_type {
            RelationType::HasMany => {
                let local_fields: Vec<String> =
                    fk_pairs.iter().map(|(l, _)| l.to_string()).collect();
                let foreign_fields: Vec<String> =
                    fk_pairs.iter().map(|(_, f)| f.to_string()).collect();
                let foreign_int_check: Vec<bool> = fk_pairs
                    .iter()
                    .map(|(_, f_name)| {
                        rel_cfg.columns.iter().any(|c| {
                            c.name == *f_name
                ...
                obj = obj.field(Field::new(
                    ...
                    move |ctx| {
                        ...
                        FieldFuture::new(async move {
                            ...
                            let app_ctx =
                                ctx.data::<std::sync::Arc<AppContext>>().map_err(|_| {
                                    async_graphql::Error::new("internal context missing")
                                })?;
            ...
            RelationType::BelongsTo => {
                let local_fields: Vec<String> =
                    fk_pairs.iter().map(|(l, _)| l.to_string()).collect();
                let related_pk_cols: Vec<String> = fk_pairs
                    .iter()
                    .map(|(_, f_name)| f_name.to_string())
                    .collect();
                let pk_int_check: Vec<bool> = fk_pairs
                    .iter()
                    .map(|(_, f_name)| {
                        rel_cfg.columns.iter().any(|c| {
                            c.name == *f_name
                ...
                obj = obj.field(Field::new(
                    ...
                    move |ctx| {
                        ...
                        FieldFuture::new(async move {
                            ...
                            let mut where_clauses = Vec::new();
                            let mut params = Vec::new();
                            for (i, ((local_f, related_f), is_int)) in local_fields
                                .iter()
                                .zip(related_pk_cols.iter())
                                .zip(pk_int_check.iter())
                                .enumerate()
                            {
                                let local_val = match &parent {
                                    Value::Object(map) => {
